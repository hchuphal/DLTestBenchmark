...
class TestRunner(Template_mixin):
    ...
    def __init__(self, stream=sys.stdout, verbosity=1, title=None, description=None):
        ...
        self.verbosity = verbosity
        if title is None:
            self.title = self.bc.CYAN+"\n ********** DL Testing Tool Benchmarking Report **********"+self.bc.END+"\n"
            #self.title = 'DL Testing Tool Benchmarking Report'
        else:
            self.title = title
    ...
    def getReportAttributes(self, result):
        ...
        if result.success_count:
            status.append(padding+self.bc.GREEN+'Tasks Passed:'+self.bc.END+' %s\n'    % result.success_count)
    ...
    def _generate_report(self, result):
        ...
        table.addTitles(["Task group/Sub-Task", "Count", "Pass", "Fail", "Error"])
        tests = '' 
